...
> **Windows 安装包（推荐）：**
> [NovaInput-Setup-2.0.8.exe](https://github.com/fuxingliang/nova-ime/releases/download/v2.0.8/NovaInput-Setup-2.0.8.exe)
...
[`v2.0.8`](https://github.com/fuxingliang/nova-ime/releases/tag/v2.0.8) — 当前在用稳定版（rime-ice 88 万条词库 + 设置面板 + 便携版 + 搜狗式拆字 + 快捷键速查窗 + 候选窗跳动/冷启动修复）
...
- **设置面板**：托盘 Nova 图标双击打开（候选数 / 字号 / 自学习 / 大字库 / 拆字 / 词库导入导出，均热生效）
- **快捷键速查**：候选窗右键 / 托盘菜单 / 设置面板按钮，三处均可打开
...
| 按键 | 状态 | 功能 |
|---|---|---|
| `Win+Space` / `Alt+Shift` | 系统 | 切换输入法 |
| `a-z` | 打拼音 | 输入拼音（支持简拼，如 `zg` → 两个） |
| `空格` | 打拼音 | 转为候选窗 |
| `Enter` | 打拼音 / 候选 | 上屏拼音原文（打英文用） |
| `Esc` | 打拼音 / 候选 | 取消输入 / 隐藏候选窗 |
| `Backspace` | 打拼音 | 删一个拼音字符 |
| `Backspace` | 候选 | 取消并隐藏候选窗 |
| `←` `→` | 打拼音 | 拼音内移动 |
| `空格` | 候选 | 选中高亮词上屏 |
| `1`-`9` | 候选 | 选第 N 个词 |
| `0` | 候选 | 进入造词模式（分段选字，空格逐段确认，Esc/继续打字母退出） |
| `;` | 候选 | 拆字：上屏第 1 字（如 两个 → 两） |
| `'` | 候选 | 拆字：上屏第 2 字（如 两个 → 个） |
| `↑` `↓` | 候选 | 移动候选词 |
| `PgUp` `PgDn`（或 `-` `=`） | 候选 | 上 / 下翻页 |
| `Home` `End` | 候选 | 候选列表顶部 / 底部 |
| `Ctrl+Delete` | 候选 | 删除选中的用户词 |
| `Ctrl+PgDn` | 候选 | 选中的词降权沉底（不再靠前） |
| `;` `.` 等标点 | 候选 | 先选中高亮词，再上屏中文标点 |
| 右键候选词 | 候选 | 复制 / 搜索 / 删除 / 降权 / 简繁切换 / 重置位置 / 快捷键 |
| 候选窗状态条 | 候选 | `简/繁`（输出）· `，/,`（标点）· `全/半`（宽度）三开关 |

> 拆字（`;`/`'`）与简繁/标点/全半角开关均可在设置面板调整，热生效无需重启。
